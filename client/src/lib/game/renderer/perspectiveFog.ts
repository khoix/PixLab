...
export class PerspectiveFog implements WorldVisibility {
  ...
  drawGround(ctx: CanvasRenderingContext2D, camera: PerspectiveCamera): void {
    ...
    const key = [camera.width, camera.height, camera.anchor.x, camera.anchor.y, camera.focalLength,
      camera.sinPitch, camera.cosPitch, camera.distance, camera.near, camera.far, this.radius, step].join(':');
    if (key !== this.key || !this.buffer) {
      ...
      if (!b) return;
      const pixels = b.createImageData(this.buffer.width, this.buffer.height);
      for (let y = 0; y < this.buffer.height; y++) for (let x = 0; x < this.buffer.width; x++) {
        ...
        pixels.data[(y * this.buffer.width + x) * 4 + 3] = Math.round(255 * (p ? 1 - this.visibilityAt(p) : 1));
      }
      b.putImageData(pixels, 0, 0); this.key = key; this.builds++;
    }
